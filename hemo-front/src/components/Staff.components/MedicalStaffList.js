...
import { getMedicalStaff, deleteMedicalStaff, updateUserProfile } from '../../api/staff';
...
const MedicalStaffList = () => {
  ...
  const fetchData = async () => {
    ...
    const result = await getMedicalStaff(apiBaseUrl, token);
    console.log('getMedicalStaff result:', result);
    if (!result?.success) {
      setErrors({ general: result?.error || 'Failed to fetch medical staff.' });
      setStaffList([]);
    } else {
  ...
  const handleDelete = async (id, nom, prenom) => {
    ...
    try {
      console.log('Calling deleteMedicalStaff with:', {
        apiBaseUrl,
        id,
        token: token.substring(0, 10) + '...'
      });
      const result = await deleteMedicalStaff(apiBaseUrl, token, id);
      console.log('deleteMedicalStaff result:', result);

      if (result && typeof result === 'object' && result.success) {
        setSuccess('Medical staff deleted successfully!');
        fetchData();
        setTimeout(() => setSuccess(null), 2000);
        setDebugInfo((prev) => [...prev, { request: { id, action: 'delete' }, response: result }]);
      } else {
        const errorMsg = result?.error || 'Failed to delete medical staff: Invalid response format.';
        setErrors({ general: errorMsg });
        setDebugInfo((prev) => [...prev, { request: { id, action: 'delete' }, response: { error: errorMsg } }]);
      }
    } catch (error) {
      const errorMsg = error.message || 'Failed to delete medical staff.';
      setErrors({ general: errorMsg });
      setDebugInfo((prev) => [...prev, { request: { id, action: 'delete' }, response: { error: errorMsg } }]);
    ...
    <div className="app-container">
      ...
      <div className="main-content">
        ...
        <div className="content-area">
          <h2 className="content-header">Medical Staff List</h2>
          <div className="staff-section">
            ...
            <div className="content-header">
              ...
              <div className="action-buttons">
                <button
                  ...
                  onClick={() => navigate('/home/staff/medical/add')}
                >
                  Add Medical Staff
                </button>
              </div>
            ...
            {staffList.length === 0 ? (
              <p className="no-data">No medical staff found.</p>
            ) : (
                      ...
                      <tr key={staff.id}>
                        ...
                        <td>
                          <button
                            ...
                            onClick={() => navigate(`/home/staff/medical/edit/${staff.id}`)}
                          >
                            Edit
                          </button>
                          <button
