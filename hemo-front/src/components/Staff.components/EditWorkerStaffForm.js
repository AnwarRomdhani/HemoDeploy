...
import { updateWorkerStaff } from '../../api/staff';
import axios from 'axios';
import Header from '../Header';
import SideMenu from '../SideMenu';
import './FormStyles.css';

const EditWorkerStaffForm = () => {
  ...
  useEffect(() => {
    const fetchStaffData = async () => {
      ...
      try {
        const response = await axios.get(`${apiBaseUrl}worker-staff/${id}/`, {
        ...
        setErrors({ general: err.response?.data?.error || 'Failed to load staff data.' });
      }
    };
    fetchStaffData();
  }, [id, apiBaseUrl]);
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await updateWorkerStaff(apiBaseUrl, token, id, formData);
    if (result.success) {
      setSuccess('Worker staff updated successfully!');
      setTimeout(() => navigate('/home/staff/worker'), 2000);
    } else {
      setErrors({ general: result.error || 'Failed to update worker staff.' });
    }
  };
        ...
        <div className="content-area">
          <div className="form-section">
            <div className="form-card">
              <h2 className="content-header">Edit Worker Staff</h2>
              ...
              <div className="form-container">
                ...
                <div className="form-actions">
                  ...
                  <button
                    ...
                    onClick={() => navigate('/home/staff/worker')}
                  >
                    Cancel
                  </button>
                </div>
...
export default EditWorkerStaffForm;