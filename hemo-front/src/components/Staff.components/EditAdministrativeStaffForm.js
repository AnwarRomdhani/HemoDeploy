...
import { updateAdministrativeStaff } from '../../api/staff';
import Header from '../Header';
import SideMenu from '../SideMenu';
import './FormStyles.css';

const EditAdministrativeStaffForm = () => {
  ...
  useEffect(() => {
    const fetchStaffData = async () => {
      ...
      const response = await fetch(`${apiBaseUrl}administrative-staff/${id}/`, {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await updateAdministrativeStaff(apiBaseUrl, token, id, formData);
    if (result.success) {
      setSuccess('Administrative staff updated successfully!');
      setTimeout(() => navigate('/home/staff/administrative'), 2000);
    } else {
      setErrors({ general: result.error || 'Failed to update administrative staff.' });
    }
  };
        ...
        <div className="content-area">
          <div className="form-section">
            <div className="form-card">
              <h2 className="content-header">Edit Administrative Staff</h2>
...
export default EditAdministrativeStaffForm;