...
import './AddMachine.css'; // Reuse AddMachine CSS for consistency
...
const EditMachine = () => {
  ...
  const fetchMachineData = async () => {
    ...
    try {
      const response = await axios.get(`${apiBaseUrl}machines/${id}/update/`, {
      ...
      setFormData({
        ...
        membrane: machine.membrane?.id || '',
        filtre: machine.filtre?.id || '',
      });
    } catch (err) {
      setErrors({ general: err.response?.data?.error || 'Failed to fetch machine data.' });
    }
  };
    ...
    try {
      await axios.put(
        `${apiBaseUrl}machines/${id}/update/`,
        {
          ...
          membrane_id: formData.membrane || null,
          filtre_id: formData.filtre || null,
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      setErrors({ general: err.response?.data?.error || 'Failed to update machine.' });
    }
  };
